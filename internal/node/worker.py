...
def protobuf(raw):
    ...
    def varint():
        ...
        for shift in range(0,70,7):
            if pos >= len(raw): raise Failure('protobuf-truncated')
            b = raw[pos]; pos += 1
            value |= (b & 127) << shift
    ...
    while pos < len(raw):
        ...
        if number == 0 or number in fields: raise Failure('protobuf-field')
        if wire == 0: value = varint()
        elif wire in [1,2,5]:
            size = varint() if wire == 2 else (8 if wire == 1 else 4)
            if size > len(raw)-pos: raise Failure('protobuf-truncated')
...
class Worker:
    def __init__(self, request, root=Path('/var/lib/dashnet'), lock=Path('/run/dashnet-bootstrap.lock')):
    ...
    def verify_instance(self):
        ...
        with self.opener.open(token_request, timeout=5) as response: token = response.read(1024).decode()
        request = urllib.request.Request('http://169.254.169.254/latest/meta-data/instance-id', headers={'X-aws-ec2-metadata-token': token})
        with self.opener.open(request, timeout=5) as response: actual = response.read(1024).decode()
    ...
    def run(self, args, stdin=None, timeout=120):
        try:
            result = subprocess.run(args, input=stdin, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=timeout, check=False)
    ...
    def docker(self, *args, timeout=120):
        return self.run(['docker', '--host', 'unix:///var/run/docker.sock', '--config', str(self.root / 'docker-config'), *args], timeout=timeout)

    def container_name(self, service): return self.project + '-' + service

    def inspect_container(self, service):
        raw = self.docker('container', 'ls', '-a', '--filter', 'name=^/' + self.container_name(service) + '$', '--format', '{{.ID}}').decode().strip()
        ...
        self.require(labels.get('dashnet.compute') == self.c['computePlanId'] and labels.get('dashnet.node') == self.t['name'] and labels.get('dashnet.network') == self.c['network'], 'container-ownership')
        return value

    def labels(self):
        return {'dashnet.compute': self.c['computePlanId'], 'dashnet.node': self.t['name'], 'dashnet.network': self.c['network']}

    def service(self, name, image):
        return dict(image=image, container_name=self.container_name(name), network_mode='host', user='0:0', restart='unless-stopped', labels=self.labels(), logging=dict(driver='local', options={'max-size':'25m','max-file':'3'}))
    ...
    def compose(self, component, services):
        ...
        if existing != value: self.atomic(component + '/compose.json', value)
        self.docker('compose', '-p', self.project, '-f', str(path), 'config', '--quiet')
        self.docker('compose', '-p', self.project, '-f', str(path), 'up', '-d', '--no-deps', '--pull', 'never', *services.keys())

    def owned(self):
        self.require(self.root.is_dir() and not self.root.is_symlink(), 'missing-bootstrap-root')
        ...
        for name in ['core', 'platform', 'secrets.json', 'data', 'transactions', 'deployment.json']:
        ...
        for line in self.docker('container','ls','-a','--format','{{.Names}}').decode().splitlines():
            self.require(line in {self.container_name(x) for x in ['core','miner','drive','tenderdash','dapi','gateway']}, 'unexpected-container')
        for service in ['core','miner','drive','tenderdash','dapi','gateway']:
            if self.inspect_container(service): pass
    ...
    def rpc(self, method, params=None, wallet=False):
        ...
        url = 'http://127.0.0.1:' + str(self.ports['coreRPC']) + ('/wallet/dashnet' if wallet else '/')
        payload = json.dumps(dict(jsonrpc='1.0', id='dashnet', method=method, params=params or [])).encode()
        auth = base64.b64encode(('dashnet:' + secret['rpcPassword']).encode()).decode()
        request = urllib.request.Request(url, data=payload, headers={'Authorization':'Basic ' + auth, 'Content-Type':'application/json'})
        try:
        ...
        data = json.loads(raw)
        if data.get('error'): raise RPCFailure(method, data['error']['code'])
        return data['result']
    ...
    def core_config(self, final=False):
        ...
        lines = ['devnet=' + self.c['coreNetwork'], 'daemon=0', 'server=1', 'txindex=1', 'addressindex=1', 'spentindex=1', 'timestampindex=1', 'dnsseed=0', 'discover=0', 'allowprivatenet=1', 'listen=1', 'maxconnections=256', 'fallbackfee=0.00001', 'rpcuser=dashnet', 'rpcpassword=' + secret['rpcPassword'], 'rpcallowip=127.0.0.1', 'rpcworkqueue=128', 'rpcthreads=32', 'deprecatedrpc=hpmn', 'llmqchainlocks=llmq_devnet', 'llmqinstantsenddip0024=llmq_devnet_dip0024', 'llmqplatform=llmq_devnet_platform', 'llmqmnhf=llmq_devnet']
        if final:
            self.require(self.q.get('sporkAddress'), 'missing-spork-address')
            lines.append('sporkaddr=' + self.q['sporkAddress'])
            if self.t['role'] == 'wallet': lines.append('sporkkey=' + secret['sporkKey'])
            if self.t['role'] == 'validator': lines.append('masternodeblsprivkey=' + secret['operatorPrivateKey'])
        lines += ['[devnet]', 'port=' + str(p['coreP2P']), 'rpcport=' + str(p['coreRPC']), 'rpcbind=127.0.0.1', 'bind=0.0.0.0', 'externalip=' + self.t['peerAddress'] + ':' + str(p['coreP2P']), 'minimumdifficultyblocks=1000000', 'highsubsidyblocks=500', 'highsubsidyfactor=100', 'powtargetspacing=10']
        for name in ['rawtx','rawtxlock','rawblock','hashblock','rawchainlocksig','rawtxlocksig']:
            lines.append('zmqpub' + name + '=tcp://127.0.0.1:' + str(p['coreZMQ']))
        lines += ['addnode=' + peer for peer in self.c['corePeers'] if peer != self.t['peerAddress'] + ':' + str(p['coreP2P'])]
        return '\n'.join(lines) + '\n'
    ...
    def ensure_core(self, final=False):
        ...
        if changed: self.atomic('core/dash.conf', config)
        (self.root / 'data/core').mkdir(parents=True, exist_ok=True, mode=0o700)
        service = self.service('core', self.images['core'])
        service.update(entrypoint=['dashd'], command=['-conf=/etc/dash/dash.conf','-datadir=/data','-printtoconsole=1'], volumes=[str(config_path) + ':/etc/dash/dash.conf:ro', str(self.root / 'data/core') + ':/data'], stop_grace_period='120s')
        ...
        while True:
            try:
                ...
                if time.monotonic() >= deadline: raise Failure('core-rpc-not-ready') from None
                time.sleep(1)
        if final: self.atomic('core/state.json', {'final':True, 'sporkAddress':self.q['sporkAddress']})
        return result
    ...
    def core_status(self):
        ...
        try: chainlock = self.rpc('getbestchainlock')['height']
            ...
            if error.code not in [-32603,-5,-8]: raise
        state, protx = '', ''
    ...
    def verify_image(self, container, pinned):
        ...
        self.require(image['Architecture'] == self.t['architecture'] and image['Os'] == 'linux', 'image-platform-drift')
    ...
    def identity(self):
        ...
        if 'operatorPrivateKey' not in secret:
            ...
            secret.update(operatorPrivateKey=pair['secret'], operatorPublicKey=pair['public'])
            ...
            secret['tlsCertificate'], secret['tlsPrivateKey'] = self.q['tlsCertificate'], self.q['tlsPrivateKey']
            self.atomic('secrets.json', secret)
        return dict(operatorPublicKey=secret['operatorPublicKey'], platformNodeId=secret['platformNodeID'])
    ...
    def wallet(self):
        ...
        if 'dashnet' not in self.rpc('listwallets'):
            ...
            if 'dashnet' in names: self.rpc('loadwallet', ['dashnet'])
            else: self.rpc('createwallet', ['dashnet'])
        secret = self.secret()
    ...
    def register(self):
        ...
        name = target['name']
        collateral, owner = self.address('dashnet:' + name + ':collateral'), self.address('dashnet:' + name + ':owner')
        ...
        for _ in range(required + 1):
            if tx.get('confirmations', 0) >= required: break
        ...
        state = info['state']
        self.require(state['pubKeyOperator'] == target['operatorPublicKey'] and state['ownerAddress'] == owner and state['platformNodeID'] == target['nodeId'], 'registration-readback-mismatch')
    ...
    def activate(self):
        ...
        for key in ['SPORK_17_QUORUM_DKG_ENABLED','SPORK_19_CHAINLOCKS_ENABLED','SPORK_21_QUORUM_ALL_CONNECTED']:
            if key in active and not active[key]: self.rpc('spork', [key, 0])
        return {}
    ...
    def mine_start(self):
        ...
        service.update(entrypoint=['/bin/sh','-c'], command=['while true; do dash-cli -conf=/etc/dash/dash.conf generatetoaddress 1 ' + address + ' 1000000 >/dev/null 2>&1; sleep 10; done'], volumes=[str(self.root / 'core/dash.conf') + ':/etc/dash/dash.conf:ro'])
        self.compose('miner', {'miner':service})
        return {}

    def immutable(self, path, value):
        previous = self.read(path)
        self.require(previous is None or previous == value, 'immutable-platform-config-changed')
        if previous is None: self.atomic(path, value)

    def platform_files(self):
        self.require(self.t['role'] == 'validator', 'validator-only')
        secret, p = self.read('secrets.json'), self.ports
        own = [v for v in self.q['peers'] if v['name'] == self.t['name']]
        self.require(len(own) == 1 and own[0]['nodeId'] == secret['platformNodeID'] and own[0]['operatorPublicKey'] == secret['operatorPublicKey'], 'platform-identity-mismatch')
        self.require(self.q['genesisCoreHeight'] > 0, 'missing-genesis-chainlock')
        genesis = dict(genesis_time=self.c['genesisTime'], chain_id=self.c['platformChainId'], initial_height='1', initial_core_chain_locked_height=self.q['genesisCoreHeight'], validator_quorum_type=107, consensus_params=dict(
            block=dict(max_bytes='2097152', max_gas='57631392000', time_iota_ms='5000'),
            evidence=dict(max_age_num_blocks='100000', max_age_duration='172800000000000', max_bytes='1048576'),
            validator=dict(pub_key_types=['bls12381']), version=dict(app_version=str(self.c['initialProtocolVersion'])),
            timeout=dict(propose='50000000000', propose_delta='5000000000', vote='10000000000', vote_delta='1000000000'),
            synchrony=dict(message_delay='70000000000', precision='1000000000'), abci=dict(recheck_tx=True)))
        self.immutable('platform/tenderdash/config/genesis.json', genesis)
        self.immutable('platform/tenderdash/config/node_key.json', dict(id=secret['platformNodeID'],priv_key=dict(type='tendermint/PrivKeyEd25519',value=secret['nodePrivateKey'])))
        peers = ','.join(v['nodeId'] + '@' + v['address'] + ':' + str(p['platformP2P']) for v in self.q['peers'] if v['name'] != self.t['name'])
        # TOML string values use JSON escaping (the common TOML basic-string subset).
        lines = ['mode = "validator"', 'moniker = ' + json.dumps(self.t['name']), 'genesis-file = "config/genesis.json"', 'node-key-file = "config/node_key.json"', 'db-dir = "data"', 'log-format = "json"', '[abci]', 'transport = "routed"', 'address = ' + json.dumps('CheckTx:grpc:127.0.0.1:' + str(p['driveGRPC']) + ',*:socket:tcp://127.0.0.1:' + str(p['driveABCI'])), '[priv-validator]', 'key-file = "data/priv_validator_key.json"', 'state-file = "data/priv_validator_state.json"', 'core-rpc-host = "127.0.0.1:' + str(p['coreRPC']) + '"', 'core-rpc-username = "dashnet"', 'core-rpc-password = ' + json.dumps(secret['rpcPassword']), '[rpc]', 'laddr = "tcp://127.0.0.1:' + str(p['platformRPC']) + '"', 'unsafe = false', '[p2p]', 'laddr = "tcp://0.0.0.0:' + str(p['platformP2P']) + '"', 'external-address = ' + json.dumps(self.t['peerAddress'] + ':' + str(p['platformP2P'])), 'persistent-peers = ' + json.dumps(peers), 'max-connections = 64', 'max-outgoing-connections = 32', '[consensus]', 'create-empty-blocks = true', 'create-empty-blocks-interval = "5s"', '[tx-index]', 'indexer = ["kv"]']
        config = '\n'.join(lines) + '\n'
        path = self.root / 'platform/tenderdash/config/config.toml'
        self.require(not path.exists() or path.read_text() == config, 'tenderdash-config-drift')
        if not path.exists(): self.atomic('platform/tenderdash/config/config.toml', config)
        for name in ['drive', 'tenderdash/data']:
            (self.root / 'platform' / name).mkdir(parents=True, exist_ok=True, mode=0o700)
    ...
    def envoy(self):
        def address(port): return dict(socket_address=dict(address='127.0.0.1',port_value=port))
        def cluster(name,port,http2=False):
            value = dict(name=name,connect_timeout='5s',type='STATIC',load_assignment=dict(cluster_name=name,endpoints=[dict(lb_endpoints=[dict(endpoint=dict(address=address(port)))])]))
            if http2: value['typed_extension_protocol_options'] = {'envoy.extensions.upstreams.http.v3.HttpProtocolOptions':{'@type':'type.googleapis.com/envoy.extensions.upstreams.http.v3.HttpProtocolOptions','explicit_http_config':{'http2_protocol_options':{}}}}
            return value
        manager = {'@type':'type.googleapis.com/envoy.extensions.filters.network.http_connection_manager.v3.HttpConnectionManager','stat_prefix':'dashnet','codec_type':'AUTO','route_config':dict(name='dapi',virtual_hosts=[dict(name='dapi',domains=['*'],routes=[dict(match=dict(prefix='/org.dash.platform.dapi.'),route=dict(cluster='grpc',timeout='120s')),dict(match=dict(prefix='/'),route=dict(cluster='json',timeout='30s'))])]),'http_filters':[{'name':'envoy.filters.http.router','typed_config':{'@type':'type.googleapis.com/envoy.extensions.filters.http.router.v3.Router'}}]}
        tls = {'name':'envoy.transport_sockets.tls','typed_config':{'@type':'type.googleapis.com/envoy.extensions.transport_sockets.tls.v3.DownstreamTlsContext','common_tls_context':{'alpn_protocols':['h2','http/1.1'],'tls_certificates':[{'certificate_chain':{'filename':'/tls/cert.pem'},'private_key':{'filename':'/tls/key.pem'}}]}}}
        return dict(static_resources=dict(listeners=[dict(name='dapi_tls',address=dict(socket_address=dict(address='0.0.0.0',port_value=self.ports['gateway'])),filter_chains=[dict(transport_socket=tls,filters=[dict(name='envoy.filters.network.http_connection_manager',typed_config=manager)])])],clusters=[cluster('grpc',self.ports['dapiGRPC'],True),cluster('json',self.ports['dapiJSON'])]))
    ...
    def platform_services(self):
        ...
        td.update(entrypoint=['tenderdash'],command=['node','--home','/tenderdash'],volumes=[str(self.root / 'platform/tenderdash') + ':/tenderdash'],stop_grace_period='120s')
        dapi = self.service('dapi',self.images['dapi'])
        dapi.update(environment=dict(DAPI_GRPC_SERVER_PORT=str(p['dapiGRPC']),DAPI_JSON_RPC_PORT=str(p['dapiJSON']),DAPI_BIND_ADDRESS='127.0.0.1',DAPI_DRIVE_URI='http://127.0.0.1:'+str(p['driveGRPC']),DAPI_TENDERDASH_URI='http://127.0.0.1:'+str(p['platformRPC']),DAPI_TENDERDASH_WEBSOCKET_URI='ws://127.0.0.1:'+str(p['platformRPC'])+'/websocket',DAPI_CORE_ZMQ_URL='tcp://127.0.0.1:'+str(p['coreZMQ']),DAPI_CORE_RPC_URL='http://127.0.0.1:'+str(p['coreRPC']),DAPI_CORE_RPC_USER='dashnet',DAPI_CORE_RPC_PASS=secret['rpcPassword'],DAPI_STATE_TRANSITION_WAIT_TIMEOUT='120000',DAPI_LOG_LEVEL='info'))
        gateway = self.service('gateway',self.images['gateway'])
        gateway.update(entrypoint=['envoy'],command=['-c','/etc/envoy/config.json','--log-level','warning'],volumes=[str(self.root / 'platform/envoy.json')+':/etc/envoy/config.json:ro',str(self.root / 'platform/tls')+':/tls:ro'])
    ...
    def tenderdash(self, method):
        with self.opener.open('http://127.0.0.1:'+str(self.ports['platformRPC'])+'/'+method,timeout=15) as response:
            value = json.loads(response.read(1024*1024))
        self.require('result' in value and 'error' not in value, 'tenderdash-rpc-failed')
        return value['result']
    ...
    def platform_status(self):
        ...
        chain, network, identity = protobuf(dapi[3]),protobuf(dapi[4]),protobuf(dapi[2])
    ...
    def execute(self):
        ...
        self.require(self.q['action'] in ['inspect','core-start','core-status','identity','wallet','core-finalize','fund','register','activate','mine-start','platform-start','platform-status','stop'], 'action-refused')
        with self.lock.open('a') as lock:
            try: fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
            except BlockingIOError: raise Failure('host-busy') from None
            self.owned()
            action = self.q['action']
            if action not in ['inspect','core-status','platform-status'] and self.read('deployment.json') is None:
                self.atomic('deployment.json', {'planId':self.c['planId']})
            self.stage = action
            result = dict(instanceId=self.t['instanceId'],planId=self.c['planId'],action=action)
            if action == 'inspect': return result
            if action in ['core-start','core-finalize']: result['core'] = self.ensure_core(action == 'core-finalize')
            elif action == 'core-status': result['core'] = self.core_status()
            elif action == 'identity': result.update(self.identity())
            elif action == 'wallet': result.update(self.wallet())
            elif action == 'fund': result.update(self.fund())
            elif action == 'register': result.update(self.register())
            elif action == 'activate': result.update(self.activate())
            elif action == 'mine-start': result.update(self.mine_start())
            elif action == 'platform-start': result.update(self.platform_start())
            elif action == 'platform-status': result['platform'] = self.platform_status()
...
def main():
    ...
    try:
        ...
        if len(raw) > 1024 * 1024: raise Failure('input-size')
        ...
        reason = str(error) if isinstance(error, Failure) else type(error).__name__.lower()
        allowed = ''.join(c for c in (stage + ':' + reason) if c in 'abcdefghijklmnopqrstuvwxyz0123456789:_-')[:120]
